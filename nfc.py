...
while continue_reading:
    ...
    if (datetime.now() - first_registered).seconds > PLAYER_2_WAIT_TIME and len(players) < 2:
        players = list()
    ...
    if PLAYER_2_WAIT_TIME * 2 < time_elapsed < 60*60*30:
        print "Player #%d won" % players.index(uid) + 1
        game_on = False
        players = list()
